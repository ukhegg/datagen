	...
	namespace limits
	{
		namespace internal
		{
			...
			struct type_scoped_limit
			{
				...
				virtual void adjust_algorithm(random_source_base& r_source, value_generation_algorithm<TValue>& alg) = 0;
			...
			public:
				...
				{
					algorith_tuner_t::apply(r_source, this->limit_, alg);
				}

				void adjust_value(random_source_base& r_source, TValue& value) override
				{
					value_tuner_t::apply(r_source, this->limit_, value);
				}
			};
		...
		struct type_scoped_limits
		{
			...
			static void unregister_type_limit(scoped_limit_t& limit)
			{
				if (registered_limits_.empty())
					throw std::runtime_error("trying to unregoster scoped limit that was not registered or already removed");
